...
// import RichTextEditor from "@components/add-resource/RichTextEditor"; // <--- REMOVED direct import
...
const DynamicRichTextEditor = dynamic(
    () => import('@components/add-resource/RichTextEditor'), // Assuming this path resolves correctly
...
const ARTICLE_MEDIA_UPLOAD_API = `${API_BASE_URL}/article/upload-editor-media`; // ADDED for consistency
...
export default function EditArticlePage() {
    ...
    const handleRichTextChange = (content) => {
        // FIX 2: Ensure content is updated in form data
    ...
    const fetchArticleData = useCallback(async () => {
        ...
        try {
            ...
            // Ensure faqEntries is an array before mapping
            const faqSource = Array.isArray(articleData.faqEntries) ? articleData.faqEntries : [];
            
            const mappedFaqEntries = faqSource.map(faq => ({
            ...
            const updatedFormData = {
                ...
                // Content
                articleContent: articleData.articleContent || '', // ✅ FIX: Loaded from API
    ...
    // Derived Destination Tab Options
    ...
    const handleSubmit = async (e, finalStatus) => {
        ...
        // destinationTab যদি ফাঁকা স্ট্রিং হয়, তবে তা payload থেকে বাদ দেওয়া উচিত
        ...
        for (const key in finalFormData) {
            ...
            // Arrays and Objects (except file) are stringified
            if (Array.isArray(finalFormData[key]) || (typeof finalFormData[key] === 'object' && finalFormData[key] !== null)) {
                ...
                // Simple values (string, number, boolean) are appended directly
            ...
            {!isFetching && (
                <form onSubmit={(e) => handleSubmit(e, formData.status)} className="space-y-6">
                    <div className="grid grid-cols-1 gap-6">
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    ...
                                    <FormSelect
                                        ...
                                        options={destinationTabOptions} // Using the derived state
                                        required={false}
                        ...
                        {/* 8. Rich Text Editor (Article Content) (UPDATED WITH Dynamic Component and Value prop) */}
                        <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700">
                            <h2 className="text-xl font-semibold mb-6 text-gray-900 dark:text-gray-100 border-b pb-2 dark:border-gray-700 flex items-center gap-2">
                                <FileText className="w-5 h-5 text-red-500" />
                                8. Content Editor
                            </h2>
                            {/* --- FIX 1 & 2: Dynamic Import (Hydration Fix) and value prop (Content Load Fix) --- */}
                            <DynamicRichTextEditor // <--- FIX: Changed to Dynamic Import
                                uploadApiUrl={ARTICLE_MEDIA_UPLOAD_API} // Using defined API constant
                                onChange={handleRichTextChange} 
                                value={formData.articleContent} // <--- FIX: Changed 'initialContent' to 'value' for proper loading
                            /> 
