...
import RichTextEditor from "../../../../components/blog-components/RichTextEditor";
...
// ❌ REMOVED: CREATE_CONTENT_API_URL
// ❌ REMOVED: SIDEBAR_ITEMS_API_URL
// ❌ REMOVED: CATEGORY_FOR_SELECTION_API_URL (কারণ ব্লগে ক্যাটেগরি লাগবে না)
...
export default function CreateContentPage() {
    ...
    const [formData, setFormData] = useState({
        ...
        // Structure
        suggestedResources: [], 
        // ❌ REMOVED: usefulGuideLinks: [{ text: '', url: '', id: Date.now() + 2 }],
    ...
    // ❌ REMOVED: handleContentTypeChange (কারণ এটি ফিক্সড 'Blog')
    ...
    // --- Guide Link Handlers (❌ REMOVED) ---
    // ❌ REMOVED: handleGuideLinkChange
    // ❌ REMOVED: addGuideLink
    // ❌ REMOVED: removeGuideLink
    ...
    useEffect(() => {
        const fetchExternalData = async () => {
            ...
            try {
                ...
                // Combine Articles (Blog-এর জন্য Article সাজেস্ট করা হবে) - ✅ UPDATED
                // articleRes.data-তে এখন অনেক আর্টিকেল থাকবে, কারণ limit=9999
                ...
                if (blogRes.success && Array.isArray(blogRes.data)) {
                    combinedResources = [...combinedResources, ...blogRes.data.map(r => ({
                        id: r.permalink || r._id, 
                        name: r.title || 'Untitled Blog',
                        type: 'Blog' 
                    }))];
                } else {
                    console.error("Blog API response format is incorrect or unsuccessful:", blogRes);
                }
    ...
    const handleSubmit = async (e, finalStatus) => {
        ...
        // ✅ NEW LOGIC: isLatest যদি false হয়, তবে সেটা payload-এ যোগ করার দরকার নেই। তবে যেহেতু চেকবক্স bool পাঠাচ্ছে, তাই রেখে দেওয়া নিরাপদ।
        // if (!finalFormData.isLatest) {
        //     delete finalFormData.isLatest; 
        // }
        ...
        for (const key in finalFormData) {
            ...
            // ✅ REMOVED: content type কন্ডিশনাল রিমুভাল
            // if (key === 'category' && finalFormData.contentType === 'Blog') continue; 
            // if (key === 'subcategory' && finalFormData.contentType === 'Article') continue; 
        ...
        try {
            // ✅ FIX: শুধুমাত্র Blog API URL ব্যবহার করা হলো
        ...
        <div className="min-h-screen bg-gray-100 dark:bg-gray-900 p-4 md:p-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                <div className="grid grid-cols-1 gap-6">
                    ...
                    {/* 9. Rich Text Editor (Article Content) (UNCHANGED) */}
                    <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700">
                        <h2 className="text-xl font-semibold mb-6 text-gray-900 dark:text-gray-100 border-b pb-2 dark:border-gray-700 flex items-center gap-2">
                            <FileText className="w-5 h-5 text-red-500" />
                            8. Content Editor
